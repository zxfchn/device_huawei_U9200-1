...
//    RETURN_WRAPPED_STREAM_OUT_COMMON_CALL(stream, get_format);
    return (audio_format_t)WRAPPED_STREAM_OUT_COMMON(stream).get_format(&WRAPPED_STREAM_OUT_COMMON(stream));
}

static int out_set_format(struct audio_stream *stream, audio_format_t format)
{
//    RETURN_WRAPPED_STREAM_OUT_COMMON_CALL(stream, set_format, format);
    return WRAPPED_STREAM_OUT_COMMON(stream).set_format(&WRAPPED_STREAM_OUT_COMMON(stream), (wrapper::audio_format_t) format);
}
...
static audio_format_t in_get_format(const struct audio_stream *stream)
{
    ...
    return (audio_format_t)WRAPPED_STREAM_IN_COMMON(stream).get_format(&WRAPPED_STREAM_IN_COMMON(stream));
}

static int in_set_format(struct audio_stream *stream, audio_format_t format)
{
    // RETURN_WRAPPED_STREAM_IN_COMMON_CALL(stream, set_format, format);
    return WRAPPED_STREAM_IN_COMMON(stream).set_format(&WRAPPED_STREAM_IN_COMMON(stream), (wrapper::audio_format_t)format);
}
...
#else
    ret = WRAPPED_DEVICE_CALL(dev, open_output_stream, handle, devices, (wrapper::audio_output_flags_t)flags, 
			      (wrapper::audio_config *)config, &WRAPPED_STREAM_OUT(out));
#endif
...
{
    RETURN_WRAPPED_DEVICE_CALL(dev, set_mode, (wrapper::audio_mode_t)mode);
}
...
{
    RETURN_WRAPPED_DEVICE_CALL(dev, get_input_buffer_size, (const wrapper::audio_config*) config);
}
#else
static size_t adev_get_input_buffer_size(const struct audio_hw_device *dev,
...
#ifndef ICS_AUDIO_BLOB
    ret = WRAPPED_DEVICE_CALL(dev, open_input_stream, handle, devices, (wrapper::audio_config *)config, 
                     ...
                     hw_device_t** device)
{
    ...
    ALOGI("name: %s", name);
    ALOGI("hwif: %s", AUDIO_HARDWARE_INTERFACE);
